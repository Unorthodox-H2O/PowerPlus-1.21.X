...
public enum PMode implements StringRepresentable {
    /**
    ...
    public static final Codec<PMode> CODEC = StringRepresentable.fromEnum(PMode::values);
    public static final IntFunction<PMode> BY_ID = ByIdMap.continuous(key -> key.id, values(), ByIdMap.OutOfBoundsStrategy.ZERO);
    public static final StreamCodec<ByteBuf, PMode> STREAM_CODEC = ByteBufCodecs.idMapper(BY_ID, v -> v.id);
    ...
    PMode(int id, String name, boolean input, boolean output, boolean force, boolean canConnect) {
        this.id = id;
        this.name = name;
        this.input = input;
        this.output = output;
